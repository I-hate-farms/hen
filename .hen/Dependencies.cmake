            ...
            if( line )
                ...
                if( line_len GREATER 0 )
                    ...
                    if( NOT line_start STREQUAL "#")
                        ...
                        if( second_col STREQUAL "<built-in>")
                            set (second_col "?")
                        endif()
                        ...
                        if(third_col)
                            list (APPEND dep_list_vala_packages ${third_col})
                        else()
                            list (APPEND dep_list_vala_packages ${first_col})
                        endif()
                        #if( second_col)
...
macro (is_vala_package_in_debian vala_package in_debian)
     ...
     set( in_debian "")
     list(FIND dep_list_vala_packages ${vala_package} index)
...
macro(get_apt_pc_packages vala_package apt_packages)
    ...
    list(FIND dep_list_vala_packages ${vala_package} index)
    set (apt_packages "")
    list(LENGTH list_apt_packages list_count)

    if( index GREATER ${list_count} OR index EQUAL ${list_count})
        message (FATAL_ERROR, "${FatalColor}Cannot find apt package for the vala package '${vala_package}'${NC}\nPlease consider ${MessageColor}updating ./hen/dependencies.list${NC}")
    else ()
        if( index GREATER -1 )
            list(GET list_apt_packages ${index} apt_pkg)
            string( REPLACE "," " " final_pkg "${apt_pkg}")
            set (apt_packages "${final_pkg}")
            message ("${vala_package} -> ${apt_pkg}")
        endif()
    endif ()
endmacro()